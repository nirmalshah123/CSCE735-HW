...
// Thread variables
pthread_t *threads = NULL; // Array of thread handles
typedef struct {
    int thread_id;
    int q;
} thread_arg_t;
...
// Barriers for synchronization between threads at each merge level
pthread_barrier_t barrier1;
pthread_barrier_t barrier2;

// No mutexes/condition variables needed for this merge sort, only barriers
...
void *thread_sort_routine(void *arg) {
    ...
    int my_list_start = my_id * np;
    int my_list_end = (my_id + 1) * np;
    int my_list_size = my_list_end - my_list_start;
    int level, i;
    int ptr_local[MAX_THREADS+1];
    // Build ptr array (start indices for each thread's sublist)
    for (i = 0; i < num_threads; i++) {
        ptr_local[i] = i * np;
    }
    ptr_local[num_threads] = list_size;

    // 1. Each thread sorts its local sublist
    qsort(&list[my_list_start], my_list_size, sizeof(int), compare_int);

    // Wait for all threads to finish local sort
    pthread_barrier_wait(&barrier1);

    // 2. Iterative merging for q levels
    for (level = 0; level < q; level++) {
        int my_blk_size = np * (1 << level);
        int my_own_blk = ((my_id >> level) << level);
        int my_own_idx = ptr_local[my_own_blk];
        int my_search_blk = ((my_id >> level) << level) ^ (1 << level);
        int my_search_idx = ptr_local[my_search_blk];
        int my_search_idx_max = my_search_idx + my_blk_size;
        int my_write_blk = ((my_id >> (level+1)) << (level+1));
        int my_write_idx = ptr_local[my_write_blk];
        int idx, my_search_count, i_write;

        // Binary search for 1st element
        if (my_search_blk > my_own_blk) {
            idx = binary_search_lt(list[ptr_local[my_id]], list, my_search_idx, my_search_idx_max);
        } else {
            idx = binary_search_le(list[ptr_local[my_id]], list, my_search_idx, my_search_idx_max);
        }
        my_search_count = idx - my_search_idx;
        i_write = my_write_idx + my_search_count + (ptr_local[my_id] - my_own_idx);
        work[i_write] = list[ptr_local[my_id]];

        // Linear search for 2nd element onwards
        int j;
        for (j = ptr_local[my_id] + 1; j < ptr_local[my_id+1]; j++) {
            if (my_search_blk > my_own_blk) {
                while ((list[j] > list[idx]) && (idx < my_search_idx_max)) {
                    idx++; my_search_count++;
                }
            } else {
                while ((list[j] >= list[idx]) && (idx < my_search_idx_max)) {
            ...
            i_write = my_write_idx + my_search_count + (j - my_own_idx);
            work[i_write] = list[j];
        }

        // Barrier 1: Wait for all threads to finish scattering
        pthread_barrier_wait(&barrier1);

        // Copy work into list for next iteration (each thread copies its own sublist)
        for (j = ptr_local[my_id]; j < ptr_local[my_id+1]; j++) {
            list[j] = work[j];
        }

        // Barrier 2: Wait for all threads to finish copying
...
//
    ...
    for (j = 0; j < num_threads; j++) {
        thread_args[j].thread_id = j;
        thread_args[j].q = q;
        pthread_create(&threads[j], NULL, thread_sort_routine, &thread_args[j]);
    }
